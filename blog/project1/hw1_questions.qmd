...
```{python}
#| collapse: true
# Perform balance tests comparing treatment and control groups across key variables
...
The results show that the treatment group had a higher donation rate (2.2%) compared to the control group (1.8%). This difference is statistically significant (t-stat = 3.38, p-value < 0.001), indicating that offering a matching grant increases the likelihood of receiving a donation. The linear regression confirms this finding, showing a treatment effect of 0.004 (0.4 percentage points), while the probit model's marginal effects also indicate a significant positive impact of the treatment.
...
The results show that there are no statistically significant differences in donation rates between the different match ratios (1:1, 2:1, and 3:1). Specifically:
- 2:1 vs 1:1 comparison: t-stat = 0.42 (p-value = 0.67)
- 3:1 vs 1:1 comparison: t-stat = -0.31 (p-value = 0.76)

The donation rates across match ratios are remarkably similar:
- 1:1 match: 2.2%
- 2:1 match: 2.3%
- 3:1 match: 2.1%

This is consistent with what Karlan and List suggest in their paper. From a behavioral perspective, this is an interesting finding. It suggests that while the presence of a match offer itself increases giving, the size of the match offer does not have a significant additional effect. This highlights that what matters most is the perception of support or urgency created by the match, not necessarily the financial efficiency of the match.
...
The analysis of donation amounts reveals that the treatment group has a higher average donation amount compared to the control group. When considering all potential donors (including non-donors), the treatment effect is positive but not statistically significant at conventional levels (t-stat = 1.62, p-value = 0.11).

When we limit the analysis to only those who made a donation, we find that the treatment group gives more on average ($45.14) compared to the control group ($41.07). However, this difference is not statistically significant in the conditional analysis, suggesting that the matching grant's primary effect is on the extensive margin (whether to donate) rather than the intensive margin (how much to donate).

From a behavioral perspective, this indicates that the matching grant primarily works by motivating more people to donate rather than by encouraging substantially larger donations from those who would have given anyway.
...
This simulation demonstrates the Central Limit Theorem by showing how the distribution of mean differences between treatment and control groups becomes increasingly normal as the sample size grows. Using our observed donation rates:

- At n=50: The distribution is somewhat irregular and skewed
- At n=200: The shape becomes more symmetric
- At n=500: A clear bell curve emerges
- At n=1000: The distribution is nearly perfectly normal

This demonstration is particularly relevant for our analysis because it justifies our use of t-tests and regression analysis, even though the underlying donation data is highly skewed (most people give nothing, while a few give large amounts).

## Conclusion

Key findings from this replication:

1. Matching grants significantly increase donation rates by 0.4 percentage points (from 1.8% to 2.2%, p < 0.001)
2. Higher match ratios (2:1 or 3:1 vs 1:1) do not yield additional benefits (all p-values > 0.65)
3. While the treatment group shows higher average donations ($45.14 vs $41.07 among donors), this difference is not statistically significant
