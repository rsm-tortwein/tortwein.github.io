...
#
#
# Define log-likelihood function (negative for minimization)
def poisson_regression_negloglikelihood(beta, Y, X):
    beta = np.array(beta)
    lambda_i = np.exp(X @ beta)
    return -np.sum(Y * (X @ beta) - lambda_i - gammaln(Y + 1))

# Get Y and X from your dataframe
Y = blueprintny_df['patents'].values

# Construct the covariate matrix X:
# Constant, age, age^2, region dummies (excluding one), customer dummy
X = blueprintny_df[['age', 'region', 'customer']].copy()
X['age_squared'] = X['age']**2

# Get dummies for region, drop first to avoid dummy variable trap
region_dummies = pd.get_dummies(X['region'], prefix='region', drop_first=True)

# Final X matrix
X_final = pd.concat([
    pd.Series(1, index=X.index, name='intercept'),
    X[['age', 'age_squared', 'customer']],
    region_dummies
], axis=1)
...
result = minimize(poisson_regression_negloglikelihood,
                  x0=initial_beta,
                  args=(Y, X_np),
                  method='BFGS',
                  options={'disp': True})
...
# Compute Hessian (inverse of the covariance matrix)
